...
class Proj_Cam_model_ablation:
    ...
    @staticmethod
    def soft_round(x):
        return Proj_Cam_model_ablation.SoftRound.apply(x)
...
#             points_proj_flat = self.proj_R.T @ (points_flat - self.proj_COP.view(3,1))
        ...
        occlusion_mask = Proj_Cam_model_ablation.HardOcclusion.apply(dot_prod, 100.0)
    ...
    def compute_shadow_mask(self, height, proj_point_dir, bias=1.0, steepness=100.0):
        ...
        shadow_mask = Proj_Cam_model_ablation.HardOcclusion.apply(-diff, steepness)
        return shadow_mask
    ...
    #     u_center = Proj_Cam_model_ablation.soft_round(u - 0.5)
    #     v_center = Proj_Cam_model_ablation.soft_round(v - 0.5)
    ...
    def soft_z_buffer(
        ...
        """
        Soft splatting renderer with optional depth-aware normalization.
    
        Args:
            projected_points: (N, 2) continuous pixel coords (u, v), float
            z_values: (N,) depth in camera coordinates
            intensity: (N,) per-point intensity (scalar)
            out_size: (W, H) output resolution
            sigma: Gaussian std
            radius_multiplier: local window radius = radius_multiplier * sigma
            steepness: sigmoid mask sharpness
            depth_aware: if True, use 1/z weighting (depth-aware); if False, plain normalized weighted average
    
        Returns:
            final_output: (H, W) rendered image
        """
        device = projected_points.device
        dtype = projected_points.dtype
        ...
        # Window setup
        window_radius = int(
            torch.ceil(torch.tensor(radius_multiplier * sigma, device=device, dtype=dtype)).item()
        )
        window_size = 2 * window_radius + 1
    
        offsets = torch.arange(-window_radius, window_radius + 1, device=device, dtype=dtype)
        rel_x, rel_y = torch.meshgrid(offsets, offsets, indexing="xy")  # (ws, ws)
    
        # Point coords
        u = projected_points[:, 0]
        v = projected_points[:, 1]
    
        # Soft pixel-center rounding (keeps gradients smoother than hard round)
        u_center = Proj_Cam_model_ablation.soft_round(u - 0.5)
        v_center = Proj_Cam_model_ablation.soft_round(v - 0.5)
    
        # Local pixel centers covered by each point
        local_x = (u_center.unsqueeze(1).unsqueeze(2) + 0.5) + rel_x.unsqueeze(0)  # (N, ws, ws)
        local_y = (v_center.unsqueeze(1).unsqueeze(2) + 0.5) + rel_y.unsqueeze(0)  # (N, ws, ws)
    
        # Distances from point projection to each local pixel center
        diff_x = local_x - u.unsqueeze(1).unsqueeze(2)
        diff_y = local_y - v.unsqueeze(1).unsqueeze(2)
        dist_sq = diff_x**2 + diff_y**2
    
        # Gaussian kernel + soft support mask
        radius_sq = (radius_multiplier * sigma) ** 2
        mask = torch.sigmoid(steepness * (radius_sq - dist_sq))
        weights = torch.exp(-0.5 * dist_sq / (sigma**2 + 1e-12)) * mask  # (N, ws, ws)
    
        # Optional depth-aware weighting
        if depth_aware:
            z = z_values + 1e-6
            intensity_weight = (intensity / z).unsqueeze(1).unsqueeze(2)  # (N,1,1)
            depth_weight = (1.0 / z).unsqueeze(1).unsqueeze(2)            # (N,1,1)
        else:
            intensity_weight = intensity.unsqueeze(1).unsqueeze(2)        # (N,1,1)
            depth_weight = torch.ones_like(z_values).unsqueeze(1).unsqueeze(2)
        # print(depth_weight, 'depth_weight')
        # print(intensity_weight, 'intensity_weight')
        local_weighted_intensity = weights * intensity_weight            # (N, ws, ws)
        local_weighted_depth = weights * depth_weight                    # (N, ws, ws)
    
        # Convert local pixel centers to integer indices
        u_idx = (local_x - 0.5).long().clamp(0, cam_res_w - 1)
        v_idx = (local_y - 0.5).long().clamp(0, cam_res_h - 1)
        ...
        flat_u_idx = u_idx.reshape(num_elems)
        flat_v_idx = v_idx.reshape(num_elems)
        flat_indices = flat_v_idx * cam_res_w + flat_u_idx  # linear index into (H*W)
    
        # Accumulate into output buffers
        final_intensity = torch.zeros(cam_res_w * cam_res_h, device=device, dtype=dtype)
        final_depth = torch.zeros(cam_res_w * cam_res_h, device=device, dtype=dtype)

        # print(flat_intensity,'flat_intensity')
        final_intensity.scatter_add_(0, flat_indices, flat_intensity)
        final_depth.scatter_add_(0, flat_indices, flat_depth)
    
        # Reshape to (H, W)
        final_intensity = final_intensity.reshape(cam_res_h, cam_res_w)
        final_depth = final_depth.reshape(cam_res_h, cam_res_w)
    ...
    #     depth_mode: str = "inv_z",   # "none" | "inv_z" | "softmin"
    #     tau: float = 0.01,           # only used when depth_mode == "softmin"
    # ):
    #     """
    #     Soft splatting renderer with optional depth weighting.
    
    #     Args:
    #         projected_points: (N, 2) continuous pixel coords (u, v), float
    #         z_values: (N,) depth in camera coordinates (positive)
    #         intensity: (N,) per-point intensity (scalar)
    #         out_size: (W, H) output resolution
    ...
    #         steepness: sigmoid mask sharpness
    #         depth_mode:
    #             - "none": plain normalized weighted average (no depth effect)
    #             - "inv_z": your original 1/z depth-aware normalization
    #             - "softmin": softmin depth weighting exp(-z/tau) (more occlusion-like)
    #         tau: temperature for softmin (same unit as z_values)
    ...
    #     # Window setup
    #     # ----------------------------
    #     window_radius = int(torch.ceil(torch.as_tensor(radius_multiplier * sigma, device=device, dtype=dtype)).item())
    #     window_size = 2 * window_radius + 1
    ...
    #     # Soft pixel-center rounding (assumes you have this method)
    #     u_center = Proj_Cam_model_ablation.soft_round(u - 0.5)
    #     v_center = Proj_Cam_model_ablation.soft_round(v - 0.5)
    ...
    #     # ----------------------------
    #     # Depth / visibility weighting
    #     # ----------------------------
    #     if depth_mode == "inv_z":
    #         z = z_values + 1e-6
    #         point_num = intensity / z          # (N,)
    #         point_den = 1.0 / z                # (N,)
    #     elif depth_mode == "softmin":
    #         # More occlusion-like: exp(-z/tau)
    #         # For numeric stability, shift by min(z) (global). If you know z is small, you can omit.
    #         z = z_values
    #         z_rel = z - z.min()
    #         a = torch.exp(-z_rel / (tau + 1e-12))  # (N,)
    #         point_num = intensity * a
    #         point_den = a
    #     elif depth_mode == "none":
    #         point_num = intensity
    #         point_den = torch.ones_like(z_values)
    #     else:
    #         raise ValueError(f"Unknown depth_mode='{depth_mode}'. Use 'none', 'inv_z', or 'softmin'.")

    #     print(point_num, 'point_num')
    #     print(point_den, 'point_den')
    #     intensity_weight = point_num.unsqueeze(1).unsqueeze(2)  # (N,1,1)
    #     depth_weight = point_den.unsqueeze(1).unsqueeze(2)      # (N,1,1)
    ...
    #     flat_intensity = local_weighted_intensity.reshape(num_elems)
    #     flat_depth = local_weighted_depth.reshape(num_elems)
    #     flat_indices = (v_idx.reshape(num_elems) * cam_res_w + u_idx.reshape(num_elems)).long()
    ...
    def hard_rasterize(self, projected_points, z_values, intensity, out_size, mode="bilinear", eps=1e-6):
        """
        Rasterize projected samples back to a 2D image without soft_z_buffer windowing.
        Returns: (H, W) 2D tensor.
    
        mode:
          - "nearest": scatter to nearest pixel (fast, piecewise-constant), uses 1/z blending (like your soft formulation)
          - "bilinear": scatter to 4 neighbors with bilinear weights, uses 1/z blending (like your soft formulation)
          - "zbuffer": strict Z-buffer (per pixel keep nearest depth), intensity taken from nearest point (no 1/z blending)
        """
        device = projected_points.device
        dtype = projected_points.dtype
        cam_res_w, cam_res_h = int(out_size[0]), int(out_size[1])
        ...
        # ----- strict z-buffer -----
        if mode == "zbuffer":
            # Assign each sample to a single pixel (nearest)
            u_idx = torch.round(u).long().clamp(0, cam_res_w - 1)
            v_idx = torch.round(v).long().clamp(0, cam_res_h - 1)
            flat_idx = v_idx * cam_res_w + u_idx  # shape (N,)
    
            # 1) per-pixel min depth
            # Need scatter_reduce_ support (PyTorch >= 1.12)
            min_depth = torch.full((cam_res_w * cam_res_h,), float("inf"), device=device, dtype=dtype)
    
            if not hasattr(min_depth, "scatter_reduce_"):
                raise RuntimeError("mode='zbuffer' requires PyTorch scatter_reduce_ (PyTorch >= 1.12).")
    
            min_depth.scatter_reduce_(
                0, flat_idx, z_values.to(dtype),
                reduce="amin", include_self=True
            )
    
            # 2) keep only samples that hit the min depth (within tolerance)
            picked_min = min_depth[flat_idx]
            keep = z_values <= (picked_min + eps)
    
            # 3) write intensity of nearest sample(s)
            out_int = torch.zeros((cam_res_w * cam_res_h,), device=device, dtype=dtype)
            out_cnt = torch.zeros((cam_res_w * cam_res_h,), device=device, dtype=dtype)
    
            out_int.scatter_add_(0, flat_idx[keep], intensity[keep].to(dtype))
            out_cnt.scatter_add_(0, flat_idx[keep], torch.ones_like(intensity[keep], dtype=dtype))
    
            out = out_int / (out_cnt + eps)
    
            # pixels with no hits -> keep 0 (since out_cnt=0)
            return out.view(cam_res_h, cam_res_w)
    
        # ----- your previous "blended" hard rasterizers -----
        denom = (z_values + 1e-6)
        w_depth = 1.0 / denom
        w_int = intensity / denom
    
        if mode == "nearest":
            u_idx = torch.round(u).long().clamp(0, cam_res_w - 1)
            v_idx = torch.round(v).long().clamp(0, cam_res_h - 1)
            flat_idx = v_idx * cam_res_w + u_idx
    
            out_int = torch.zeros(cam_res_w * cam_res_h, device=device, dtype=dtype)
            out_dep = torch.zeros(cam_res_w * cam_res_h, device=device, dtype=dtype)
    
            out_int.scatter_add_(0, flat_idx, w_int)
            out_dep.scatter_add_(0, flat_idx, w_depth)
    
            out_int = out_int.view(cam_res_h, cam_res_w)
            out_dep = out_dep.view(cam_res_h, cam_res_w)
            return out_int / (out_dep + 1e-6)
            ...
            fu = (u - u0.to(dtype)).clamp(0, 1)
            fv = (v - v0.to(dtype)).clamp(0, 1)
    
            u0c = u0.clamp(0, cam_res_w - 1)
            u1c = u1.clamp(0, cam_res_w - 1)
            v0c = v0.clamp(0, cam_res_h - 1)
            v1c = v1.clamp(0, cam_res_h - 1)
            ...
            idx00 = v0c * cam_res_w + u0c
            idx10 = v0c * cam_res_w + u1c
            idx01 = v1c * cam_res_w + u0c
            idx11 = v1c * cam_res_w + u1c
    
            out_int = torch.zeros(cam_res_w * cam_res_h, device=device, dtype=dtype)
            out_dep = torch.zeros(cam_res_w * cam_res_h, device=device, dtype=dtype)
    
            out_int.scatter_add_(0, idx00, w_int * w00)
            out_int.scatter_add_(0, idx10, w_int * w10)
            out_int.scatter_add_(0, idx01, w_int * w01)
            out_int.scatter_add_(0, idx11, w_int * w11)
    
            out_dep.scatter_add_(0, idx00, w_depth * w00)
            out_dep.scatter_add_(0, idx10, w_depth * w10)
            out_dep.scatter_add_(0, idx01, w_depth * w01)
            out_dep.scatter_add_(0, idx11, w_depth * w11)
            ...
            raise ValueError(f"Unknown mode={mode}. Use 'nearest', 'bilinear', or 'zbuffer'.")
    def render(
        self,
        height,
        reflectance,
        pattern,
        extent_mm=[-100, 100, -100, 100],
        shade_on=True,
        fov_mask_on=True,
        proj_point_dir=None,
        cam_point_dir=None,
        ...
        show=False,
        print_matrix=False,
        use_soft_z_buffer=False,
        hard_mode='nearest',
        sigma_softz=0.5,
        radius_multi_softz=2.0,
        steepness_softz=200,
        depth_aware=1,                # ✅ 新增参数
    ):
        ...
        image_values = self.projector_projection(
            points_3d,
            pattern,
            reflectance,
            proj_point_dir,
            shade_on=shade_on,
            fov_mask_on=fov_mask_on,
        )
        ...
        if use_soft_z_buffer:
            observed_image = self.soft_z_buffer(
                valid_points,
                valid_z,
                valid_intensity,
                out_size,
                sigma=sigma_softz,
                radius_multiplier=radius_multi_softz,
                steepness=steepness_softz,
                depth_aware=depth_aware,      # ✅ 传入开关
            )
        else:
            observed_image = self.hard_rasterize(
                valid_points,
                valid_z,
                valid_intensity,
                out_size,
