...
#             points_proj_flat = self.proj_R.T @ (points_flat - self.proj_COP.view(3,1))
    ...
    def render(self, height, reflectance, pattern,
           ...
           use_soft_z_buffer=False, hard_mode = 'nearest',
           sigma_softz = 0.5, radius_multi_softz = 2.0, steepness_softz = 200
              ):
        ...
        if use_soft_z_buffer:
            observed_image = self.soft_z_buffer(
                valid_points, valid_z, valid_intensity,
                out_size, sigma = sigma_softz, radius_multiplier=radius_multi_softz, steepness = steepness_softz
            )
        else:
            # IMPORTANT: output is 2D (H, W), not flat
            # observed_image = self.hard_rasterize(
            #     valid_points, valid_z, valid_intensity,
            #     out_size, mode=hard_mode
            # )
            observed_image = self.hard_rasterize(
                valid_points, valid_z, valid_intensity,
                out_size, mode = hard_mode)
